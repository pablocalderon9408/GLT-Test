...
def signup(user: SignUpUser):
    ...
    Returns:
        JSONResponse: A JSON response indicating if the registration
        was successful or not.
    """
    if user.password != user.password_confirmation:
        return JSONResponse(
            status_code=400,
            content={"message": "Las contraseñas no coinciden"}
            )
...
def validate(token: str):
    """
    Description: Validates a user account using a token
    received via email.

    Request Parameters:
    - token: string, required. The token received via email
    for validating the user account.

    Response:
    - If the token is invalid or expired: status code 401,
    message: "El token no es válido".
    - If the user account does not exist: status code 404,
    message: "El usuario no existe".
    - If the validation is successful: status code 200,
    ...
    if not decoded_token:
        return JSONResponse(
            status_code=401,
            content={"message": "El token no es válido"}
            )
    ...
    return JSONResponse(
        status_code=200,
        content={"message": "El usuario ha sido validado"}
        )
...
def login(user: LoginUser):
    ...
    Returns:
        JSONResponse: A token if the user is authorized,
...
def delete_user(id: int = Path(ge=1, le=2000)) -> SignUpUser:
    ...
    Returns:
    - A JSON response with the message "Se ha eliminado el usuario."
    (The user has been deleted.)

    Raises:
    - HTTPException(404): If the user with the given ID does not
