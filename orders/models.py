...
class Order(BaseModel):
    ...
    user_id : int
        Foreign key referencing the id of the user who
        placed the order.
    user : User
        Many-to-one relationship with the User model,
        representing the user who placed the order.
    products : List[OrderProducts]
        One-to-many relationship with the OrderProducts model,
        representing the products associated with the order.
    """
...
class OrderProducts(BaseModel):
    ...
    order_id : int
        Foreign key referencing the id of the order
        associated with the product.
    product_id : int
        Foreign key referencing the id of the product
        associated with the order.
    quantity : int
        Quantity of the product associated with the order.
    product : Product
        Many-to-one relationship with the Product model,
        representing the product associated with the order.
    order : Order
        Many-to-one relationship with the Order model,
        representing the order associated with the product.
    """
